...
def plot_images(data: list[dict], document: pymupdf.Document) -> None:
    images = []
    print(data)
    ...
    for item in data:
        ...
        img_type = item['type']
        page = item['page_indices'][0] - 1
        coords = item['coordinates'] * 0.179
        pix, img = extract_image(document[page], coords)
        images.append((img, img_type))
        print(img.shape)
    ...
    fig.tight_layout()
    try:
        for ax, (img, img_type) in zip(axs, images):
            ax.imshow(img)
            ax.set_title(img_type)
            ax.axis('off')
    except:
        img, img_type = images[0]
        axs.imshow(img)
        axs.set_title(img_type)
        axs.axis('off')
